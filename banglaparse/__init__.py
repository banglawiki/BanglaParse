...
class Tokenizer(object):
    ...
    def initialize(self, dictionary=None):
        ...
        with self.lock:
            ...
            # default dictionary
                ...
                cache_file = "banglaparse.cache"
            # custom dictionary
            else:
                cache_file = "banglaparse.u%s.cache" % md5(
    ...
    def cut(self, sentence, cut_all=False, HMM=True):
        ...
        Bengali characters into separated words.
    ...
    def tokenize(self, unicode_sentence, mode="default", HMM=True):
        ...
        if not isinstance(unicode_sentence, text_type):
            raise ValueError("banglaparse: the input parameter should be unicode.")
        start = 0
        if mode == 'default':
...
def enable_parallel(processnum=None):
    ...
    if os.name == 'nt':
        raise NotImplementedError(
            "banglaparse: parallel mode only supports posix system")
    else:
